...
public enum YRPageViewTransformerType: Int {
    case crossFading
    case zoomOut
    case depth
    case overlap
...
open class YRPageViewTransformer: NSObject {
    open internal(set) weak var pagerView: YRPageView?
    open internal(set) var type: YRPageViewTransformerType
    
    open var minimumScale: CGFloat = 0.65
    open var minimumAlpha: CGFloat = 0.6
    
    public init(type: YRPageViewTransformerType) {
        self.type = type
        switch type {
        case .zoomOut:
    ...
    open func applyTransform(to attributes: YRPageViewLayoutAttributes) {
