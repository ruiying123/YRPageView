...
class YRPageViewLayout: UICollectionViewLayout {

    internal var contentSize: CGSize = .zero
    internal var leadingSpacing: CGFloat = 0
    internal var itemSpacing: CGFloat = 0
    internal var needsReprepare = true
    internal var scrollDirection: YRPageViewScrollDirection = .horizontal
    
    open override class var layoutAttributesClass: AnyClass {
        get {
            return YRPageViewLayoutAttributes.self
        }
    }
    
    fileprivate var pagerView: YRPageView? {
        return self.collectionView?.superview?.superview as? YRPageView
    }
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        while origin <= maxPosition {
            ...
            let attributes = self.layoutAttributesForItem(at: indexPath) as! YRPageViewLayoutAttributes
    ...
    override open func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        let attributes = YRPageViewLayoutAttributes(forCellWith: indexPath)
    ...
    fileprivate func applyTransform(to attributes: YRPageViewLayoutAttributes, with transformer: YRPageViewTransformer?) {
        guard let collectionView = self.collectionView else {
            return
        }
